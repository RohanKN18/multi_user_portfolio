...
router.get("/addproject", isLoggedIn, async (req, res) => {
  const [footer, projects] = await Promise.all([
...
router.get("/:projectId/edit", isLoggedIn, async (req, res) => {
  try {
    ...
    res.render("admin/project/editproject.ejs", { project, footer, projects, user: req.user });
  } catch (err) {
...
router.post("/:projectId/edit", isLoggedIn, async (req, res) => {
  try {
    ...
    // ✅ FIXED redirect
    return res.redirect(
      `/${req.user.username}/portfolio/projects/${project._id}`,
    );
  } catch (err) {
...
router.get("/:projectId", isLoggedIn, async (req, res) => {
  try {
    ...
    res.render("admin/projectindetail.ejs", { project, footer, projects, user: req.user });
  } catch (err) {
    console.log(err);
    res.status(500).send("Server error");
  }
